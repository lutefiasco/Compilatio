#!/usr/bin/env python3
"""
Fix Trinity College Cambridge manuscript thumbnails in Compilatio database.

The TCC importer looked for thumbnails in the first canvas, but TCC manifests
have thumbnails at the manifest level. This script fetches the correct
thumbnail URL from each manifest's top-level thumbnail field.

Features:
- Batch processing with commits every N manuscripts (default 10)
- Progress tracking file for resume capability
- Automatic resume from where it left off
- Retry logic for failed requests

Usage:
    python scripts/fix_tcc_thumbnails.py              # Dry-run
    python scripts/fix_tcc_thumbnails.py --execute    # Apply changes
    python scripts/fix_tcc_thumbnails.py --limit 10   # Test with 10 manuscripts
    python scripts/fix_tcc_thumbnails.py --reset      # Clear progress and start fresh
    python scripts/fix_tcc_thumbnails.py --status     # Show current progress
"""
...
PROGRESS_FILE = PROJECT_ROOT / "scripts" / ".tcc_thumbnail_progress.json"

REQUEST_DELAY = 0.5  # seconds between requests
BATCH_SIZE = 10  # commit and save progress every N manuscripts
MAX_RETRIES = 3  # retry failed requests
REQUEST_TIMEOUT = 60  # seconds to wait for each request
...
def fetch_thumbnail_from_manifest(manifest_url: str, retries: int = MAX_RETRIES) -> str | None:
    """
    Fetch the thumbnail URL from a TCC IIIF manifest.

    TCC manifests use IIIF Presentation 2.x format with thumbnail at the manifest level.
    ...
    for attempt in range(retries):
        try:
            ...
            # TCC manifests have thumbnail at manifest level (not canvas level)
            # IIIF Presentation 2.x format: thumbnail is a string URL or object with @id
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix TCC thumbnail URLs by fetching from IIIF manifests"
    )
    parser.add_argument(
        '--execute',
        action='store_true',
    ...
    # Get all TCC manuscripts with manifests
    query = """
    ...
    print(f"Total TCC manuscripts: {total}")
